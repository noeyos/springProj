...
@Slf4j
// memberVO 정보를 User(스프링 시큐리티에서 제공해준느 사용자 정보 클래스) 객체 정보에 연계해서 넣어줌
...
public class CustomUser extends User {
    ...
    // username(PK) = USER_NO(PK)
    // password = USER_PW
    //Collection<? extends GrantedAuthority> = AUTH(MEMBER_AUTH)
    ...
    public CustomUser(MemberVO memberVO) {
        ...
        // 회원 정보를 보내 스프링이 인증/인가 처리 및 관리를 하게 됨
        super(memberVO.getUserNo()+"", memberVO.getUserPw(),
                memberVO.getMemberAuthVOList().stream().map
