...
public class SecurityController {
    ...
    //회원게시판의 등록
    //로그인한 회원 중 ROLE_ADMIN 또는 ROLE_MEMBER 권한을 갖는 경우 접근 가능
    // 1) @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_MEMBER')")
    // 2) @Secured({"ROLE_ADMIN", "ROLE_MEMBER"})
    // 3) @PreAuthorize("hasRole('ROLE_MEMBER') or hasRole('ROLE_ADMIN')")
    ...
    //요청URI : /notice/list
    //공지사항 게시판의 목록
    //누구나 접근 가능 -> ROLE_MEMBER 또는 ROLE_ADMIN 권한에 상관없이 로그인 했다면 접근 가능
