...
public class ModelController {
	...
	//2. 모델을 통한 데이터 전달
	// Model 객체를 통해서 다양한 데이터를 뷰(View, jsp)에 전달할 수 있음
	
	//2-1) 매개변수에 선언된 Model 객체의 addAttribute() 메서드를 호출하여
	// 데이터를 뷰(View, jsp)에 전달
	//요청URI : /model/read01
	@GetMapping("/read01")
	...
	//2-2) Model 객체에 자바빈즈 클래스(MemberVO_backup) 객체(memberVO)를 값으로만 전달 시
	//		뷰(jsp)에서 참조할 이름을 클래스명(MemberVO_backup)의 앞글자를 소문자로 변환(memberVO)하여 처리함
	@GetMapping("/read02")
	public String read02(Model model) {
		MemberVO_backup memberVOBackup = new MemberVO_backup();
		...
		memberVOBackup.setDateOfBirth(cal.getTime());
		//model.addAttribute("memberVOBackup",memberVOBackup);
		model.addAttribute(memberVOBackup);
	...
	public String read03(Model model) {
		...
		model.addAttribute("user", memberVOBackup);
		//forwarding
		return "model/read03";
	}
	...
	요청방식 : post
	 */
	//매개변수로 선언 후 forwarding 시 기본적으로 데이터 전달이 안 됨
	...
	/*
	 요청URI : /model/register02
	요청파라미터 : {userId=gaeddongi,password=java}
	요청방식 : post
	 */
	//매개변수에 골뱅이ModelAttribute 애너테이션을 사용하면 
	...
	public String register04(@ModelAttribute MemberVO_backup memberVOBackup) {
